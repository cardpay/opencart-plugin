...
function ulMcc (value) {
  ...
  value = value.replace(/^(\d{4})\s(\d{4})\s(\d{4})\s(\d{4})(\d)/g,
    '$1 $2 $3 $4 $5')
  return value
}
...
function ulValidateMonthYear () {
  const date = document.getElementById('ul-card-expiration-date').
    value.
    split('/')
...
const displayCardBrand = function () {
  ...
  if (cardNumberInputField === null || typeof cardNumberInputField ===
      'undefined') {
    return true
  }
  ...
  for (
    let cardBrandIndex = 0; cardBrandIndex <= CARD_BRANDS.length -
                            1; cardBrandIndex++
  ) {
    const cardBrand = CARD_BRANDS[cardBrandIndex]
    if (cardBrand.pattern.test(cardNumber)) {
      if (!cardBrand.cnLength.includes(cardNumber.length) ||
          !isUlLuhnAlgorithmPassed(cardNumber)) {
